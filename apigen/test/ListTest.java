...
import test.list.types.Modules2;
...
public class ListTest {
    ...
    public void run() {
        ...
        mods[0] =
            factory.ModulesFromTerm(
                factory.getPureFactory().parse("[\"m1\",\"m2\",\"m3\",\"m4\"]"));
        ...
        testAssert(
            "[\"amodule\",\"amodule\",\"amodule\"]".equals(strs[3]),
            "toStringTest3");
        ...
        testAssert(
            "list([\"amodule\",\"amodule\",\"amodule\"])".equals(strs[4]),
            "toStringTest4");
        ...
        Factory f = mempty.getListFactory();
        ...
        try {
            ...
            testAssert(
                false,
                "ClassCastException occurred due to wrong equivalence implementation");
        }
        ...
        testAssert(
            sep.reverseSeparated().toTerm().isEqual(patternReversed),
            "separated reverse toTerm test");

        Module head = sep.getHead();

        testAssert(
            head.isEqual(factory.makeModule_Default("m1")),
            "separator from term test, head");
        ...
        testAssert(
            second.isEqual(factory.makeModule_Default("m2")),
            "separator from term test, second element");
        ...
        testAssert(
            sep.toTerm().isEqual(factory.getPureFactory().makeList()),
            "empty separated list");

        m = factory.makeModule_Default("m");
        sep = factory.makeSeparated(m);

        testAssert(
            sep.toTerm().isEqual(factory.getPureFactory().makeList(m.toTerm())),
            "singleton separated list");
        ...
        ATerm p =
            factory.getPureFactory().parse("[\"m\",l(\"l1\"),\"sep\",l(\"l2\"),\"m2\"]");
        testAssert(sep.toTerm().isEqual(p), "many separated list");

        ATerm pc =
            ((ATermList) p)
                .concat(
                    (ATermList) factory.getPureFactory().parse(
                        "[l(\"l1\"),\"sep\",l(\"l2\")]"))
                .concat((ATermList) p);
    ...
    private void testNineSeps() {
        ...
        testAssert(
            ns.toTerm().isEqual(
                factory.getPureFactory().parse("[\"m\",1,2,3,4,5,6,7,8,9,\"m2\"]")),
            "many separated toTerm");
        testAssert(
            ns.reverse().isEqual(factory.makeNineSeps(m2, factory.makeNineSeps(m))),
            "many separated reverse");
        ...
        testAssert(
            ns2.isEqual(
                factory.NineSepsFromString(
                    "[\"m\",1,2,3,4,5,6,7,8,9,\"m2\",1,2,3,4,5,6,7,8,9,\"m2\"]")),
            "test append");
    }
