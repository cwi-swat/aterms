...
ADTsubstitute(ADTEntries entries, ADTSubstFunc substitute, void *user_data)
{
  ATermList todo  = (ATermList)ADTmakeTermFromEntries(entries);
  ATermList done = ATempty;

  while (!ATisEmpty(todo)) {
    ADTEntry before = ADTmakeEntryFromTerm(ATgetFirst(todo));
    ADTEntry after  = substitute(before, user_data);

    if (after != NULL) {
      done = ATinsert(done, ADTmakeTermFromEntry(after));
    }

    todo = ATgetNext(todo);
  }

  return ADTmakeEntriesFromTerm((ATerm)ATreverse(done));
}
