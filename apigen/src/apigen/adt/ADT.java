...
public class ADT {
	...
	public static ADT getInstance() {
		if (instance == null) {
			throw new RuntimeException(
					"ADT.getInstance called before initialize");
		}
		return instance;
	}
	...
	private ADT(Modules adt) throws ADTException {
		...
		while (!adt.isEmpty()) { // For each module
			...
			while (!entries.isEmpty()) { // For each entry in the module
				...
				alts.add(first);
				String typeId = ((ATermAppl) first.getSort()).getAFun()
						.getName();
				...
				while (iter.hasNext()) {
					try {
						...
						if (((ATermAppl) entry.getSort()).getAFun().getName()
								.equals(typeId)) {
							alts.add(entry);
							iter.remove();
						}
					} catch (ClassCastException exc) {
						throw new ADTException("unexpected entry found:"
								+ iter.previous());
					}
				}

				listModuleType.add(processAlternatives(typeId.toString(),
						moduleName, alts));
			}
		...
		/*
		 * System.out.println("Computed Types"); Iterator iter = typeIterator();
		 * while(iter.hasNext()) { System.out.println(iter.next()); }
		 * System.out.println("Computed BottomTypes"); iter =
		 * bottomTypeIterator(); while(iter.hasNext()) {
	...
	/**
	 * Analyse the list of module to see whether there are no cycles and module
	 * with same names
	 */
	...
	private Type processAlternatives(String typeId, String moduleName, List<Entry> alts) {
		...
		if (first.isList() || first.isSeparatedList() || first.isNamedList()) {
			if (alts.size() > 1) {
				throw new RuntimeException(
						"Multiple definitions of same list: " + alts);
			}
	...
	private Type processSeparatedList(String typeId, String moduleName,
			Entry entry) {
		String elementType = ((ATermAppl) entry.getElemSort()).getAFun()
				.getName();
		Separators separators = entry.getSeparators();
		SeparatedListType type = new SeparatedListType(typeId, moduleName,
				elementType, separators, factory);
	...
	private Type processNamedList(String typeId, String moduleName, Entry entry) {
		...
		NamedListType type = new NamedListType(typeId, moduleName, elementType,
				opName, factory);
		type.addAlternatives();
		types.add(type);
		return type;
	...
	private Type processList(String typeId, String moduleName, Entry first) {
		String elementType = ((ATermAppl) first.getElemSort()).getAFun()
				.getName();
	...
	private Type processConstructors(String typeId, String moduleName, List<Entry> alts) {
		...
		while (iter.hasNext()) {
			...
			if (entry.isConstructor()) {
				String altId = ((ATermAppl) entry.getAlternative()).getAFun()
						.getName();
	...
	private void addAlternative(String typeId, Type type, String altId,
			Alternative alt) {
		if (!type.hasAlternative(altId)) {
			type.addAlternative(alt);
		} else {
			throw new RuntimeException(altId
					+ " is defined more than once for " + typeId);
		}
	}
	...
	public Iterator<Type> typeIterator(Module module) {
		return modulesTypes.get(module.getModulename().getName())
				.iterator();
	}
