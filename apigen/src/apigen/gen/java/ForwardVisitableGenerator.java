...
public class ForwardVisitableGenerator extends JavaGenerator {
	...
	public ForwardVisitableGenerator(ADT adt, JavaGenerationParameters params,
			Module module) {
		super(params);
		this.module = module;
	...
	protected void generate() {
		...
		println("public class " + getClassName() + " extends "
				+ module.getModulename().getName() + "Fwd implements "
				+ traveler + ".reflective.VisitableVisitor {");
		...
		println("  }");
		println();
		println("  public " + traveler + ".Visitable setChildAt(int i, "
				+ traveler + ".Visitable child) {");
		println("    switch (i) {");
		println("    case 0: any = (" + traveler
				+ ".reflective.VisitableVisitor) child; return this;");
		println("    default: throw new IndexOutOfBoundsException();");
		println("    }");
		println("  }");
		println();
		println("  public " + getClassName() + "(" + traveler
				+ ".reflective.VisitableVisitor any) {");
		println("    super(any);");
		println("  }");
		println("}");
	}
