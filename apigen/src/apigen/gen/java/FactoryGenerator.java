...
public class FactoryGenerator extends JavaGenerator {
	...
	private void genByteToChar() {
		...
		println("      return getPureFactory().makeInt(ch);");
		println("  }");
		println();
	}
	...
	private void genTypeFromTermMethods(ADT api, boolean forwarding) {
		Set<String> moduleToGen = new HashSet<String>();
		if (!forwarding) {
			moduleToGen.add(module.getModulename().getName());
		} else {
			moduleToGen = adt.getImportsClosureForModule(module.getModulename()
					.getName());
	...
	private void genTypeFromMethods(ADT api, boolean forwarding) {
		...
		if (!forwarding) {
			...
			moduleToGen = api.getImportsClosureForModule(module.getModulename()
					.getName());
			moduleToGen.remove(module.getModulename().getName());
		}
		Iterator<String> moduleIt = moduleToGen.iterator();
	...
	private void genConstructor() {
		...
		println();
		println("  public synchronized static " + getClassName()
				+ " getInstance(aterm.pure.PureFactory factory) {");
		...
		println("        throw new RuntimeException(\"Don't create two "
				+ getClassName()
				+ " factories with differents PureFactory \");");
		println("    }");
			...
			while (types.hasNext()) {
				...
				if (type instanceof ListType) {
					...
					if (type instanceof SeparatedListType) {
						...
						genMakeFixedSizedList(typeName, methodName, listType,
								forwarding, moduleName);
						genReverseLists(listType, methodName, forwarding,
								moduleName);
						genConcatLists(listType, methodName, forwarding,
								moduleName);
						genAppendLists(listType, methodName, forwarding,
								moduleName);
					}
				}
			}
	...
	private void genAppendLists(ListType type, String makeMethodName,
		...
		String className = TypeGenerator.qualifiedClassName(params, type
				.getId());
		String elementTypeName = TypeGenerator.qualifiedClassName(params, type
				.getElementType());

		println("  public " + className + " append" + "(" + className
				+ " list, " + elementTypeName + " elem) {");
		if (!forwarding) {
	...
	private void genConcatLists(ListType type, String makeMethodName,
		...
		println("  public " + className + " concat(" + className + " arg0, "
				+ className + " arg1) {");
		if (!forwarding) {
	...
	private void genReverseLists(ListType type, String makeMethodName,
		...
		if (!forwarding) {
			...
			println("      reversed = " + makeMethodName
					+ "(arg.getHead(), reversed);");
	...
	private void genMakeFixedSizedList(String returnTypeName,
			String methodName, ListType type, boolean forwarding,
			String moduleName) {
		for (int i = 2; i < 7; i++) {
	...
	private void genMakeFixedSizedList(String returnTypeName,
			String methodName, ListType type, int size, boolean forwarding,
			String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String qualifiedElementName = TypeGenerator.qualifiedClassName(params,
				type.getElementType());
		String formalElems = buildFormalArgumentList(qualifiedElementName,
				"elem", size);
		String actualElems = buildActualArgumentList("elem", 1, size);
		println("  public " + returnTypeName + " " + methodName + "("
				+ formalElems + ") {");
		if (!forwarding) {
			println("    return " + methodName + "(elem0, " + methodName + "("
					+ actualElems + "));");
		} else {// forwarding
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ "." + methodName + "(elem0, " + actualElems + ");");
		}
		println("  }");
		println();
	}

	private void genMakeFixedSizedSeparatedList(String returnTypeName,
			String methodName, SeparatedListType type, boolean forwarding,
			String moduleName) {
		for (int i = 2; i < 7; i++) {
	...
	private void genMakeFixedSizedSeparatedList(String returnTypeName,
			String methodName, SeparatedListType type, int size,
			boolean forwarding, String moduleName) {
		...
		if (forwarding) {
			methodName = FactoryGenerator.className(moduleName).toLowerCase()
					+ "." + methodName;
		}
		println("    return " + methodName + "(elem0, " + actualSeps
				+ methodName + "(" + recursiveActualSeps + actualElems + "));");
	...
	private void genAppendSeparatedLists(SeparatedListType type,
			String makeMethodName, boolean forwarding, String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String qualifiedClassName = TypeGenerator.qualifiedClassName(params,
				type.getId());
		...
		println("  public " + qualifiedClassName + " append" + className + "("
				+ qualifiedClassName + " list, " + formalSeps + elementTypeName
				+ " elem) {");
		if (!forwarding) {
			println("    return concat(list, " + actualSeps + makeMethodName
					+ "(elem));");
		} else {// forwarding
			println("   return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ ".append" + className + "(list, " + actualSeps
					+ " elem);");
		}
		println("  }");
	}

	private String buildActualSeparatorArgumentList(SeparatedListType type,
			boolean convert) {
		return buildActualTypedAltArgumentList(type.separatorFieldIterator(),
				convert);
	}
	...
	private void genConcatSeparatedLists(SeparatedListType type,
			String makeMethodName, boolean forwarding, String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String className = TypeGenerator.qualifiedClassName(params, type
				.getId());
		...
		if (!forwarding) {
			...
			println("    }");
			println();
			println("    return " + makeMethodName + "(list.getHead(), "
					+ sepArgs + "result);");
		} else {// forwarding
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ ".concat(arg0, " + sepArgs + " arg1);");
		}
		println("  }");
		println();
	}

	private void genReverseSeparatedLists(SeparatedListType type,
			String makeMethodName, boolean forwarding, String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String className = TypeGenerator.qualifiedClassName(params, type
				.getId());
		...
		if (!forwarding) {
			...
			println("    " + className + "[] nodes = new " + className
					+ "[length];");
			println();
			...
			println("    }");
			println();
			println("    " + className + " result = " + makeMethodName
					+ "(nodes[0].getHead());");
			println();
			println("    for (int i = 1; i < length; i++) {");
			String elementType = TypeGenerator.qualifiedClassName(params, type
					.getElementType());
			...
			println("      result = " + makeMethodName + "(_head, " + seps
					+ "result);");
			println("    }");
			println();
			println("    return result;");
	...
	private void genMakeManyTermList(String returnTypeName, String methodName) {
		println("  public "
				+ returnTypeName
				+ ' '
				+ methodName
				+ "(aterm.ATerm head, aterm.ATermList tail, aterm.ATermList annos) {");
		println("      return (" + returnTypeName + ") factory.build(new "
				+ returnTypeName + "(this, annos, head, tail));");
		println("  }");
		println();
	}

	private void genMakeManyList(String paramType, String returnTypeName,
			String methodName, String paramTypeName, boolean forwarding,
			String moduleName) {
		String head;
		...
		println("  public " + returnTypeName + ' ' + methodName + '('
				+ paramTypeName + " head, " + returnTypeName + " tail) {");
		if (!forwarding) {
			println("    return " + methodName + "(" + head
					+ ", tail, factory.getEmpty());");
		} else {// forwarding
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ "." + methodName + "(head, tail);");
		}
		println("  }");
		println();
	}

	private void genMakeManySeparatedList(String paramType,
			String returnTypeName, String methodName, String paramTypeName,
			SeparatedListType type, boolean forwarding, String moduleName) {
		String formalSeps = buildFormalSeparatorArguments(type);
		String actualSeps = buildActualTypedAltArgumentListNoConversion(type
	...
	private void genMakeManySeparatedTermList(String returnTypeName,
			String methodName, SeparatedListType type) {
		String formalSeps = buildFormalSeparatorArguments(type);
		String actualSeps = buildActualTypedAltArgumentListNoConversion(type
				.separatorFieldIterator());
		...
		println("  public " + returnTypeName + ' ' + methodName
				+ "(aterm.ATerm head, " + formalSeps
				+ "aterm.ATermList tail, aterm.ATermList annos) {");
		println("    return (" + returnTypeName + ") factory.build(new "
				+ returnTypeName + "(this, annos, head, " + actualSeps
				+ "tail));");
		println("  }");
		println();
	}

	private void genMakeSingletonList(String returnTypeName, String methodName,
			String paramTypeName, String empty, boolean forwarding,
			String moduleName) {
		println("  public " + returnTypeName + ' ' + methodName + '('
				+ paramTypeName + " elem) {");
		if (!forwarding) {
			println("    return " + methodName + "(elem, " + empty + ");");
		} else {// forwarding
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ "." + methodName + "(elem);");
	...
	private void genMakeSingletonSeparatedList(String returnTypeName,
			String methodName, String paramTypeName, SeparatedListType type,
			String empty, boolean forwarding, String moduleName) {
		String separators = buildActualNullArgumentList(type
				.separatorFieldIterator());
	...
	private void genMakeEmptyList(String returnTypeName, String methodName,
			String empty, boolean forwarding, String moduleName) {
		println("  public " + returnTypeName + ' ' + methodName + "() {");
		if (!forwarding) {
	...
	private void genFactoryForImportedModules() {
		...
		importedModules = adt.getImportsClosureForModule(module.getModulename()
				.getName());
		importedModules.remove(module.getModulename().getName());

		Iterator<String> it = importedModules.iterator();
		String moduleName;
		while (it.hasNext()) {
			moduleName = it.next();
			println("  private "
					+ FactoryGenerator.qualifiedClassName(
	...
	private void genAlternativeMembers(Type type, Alternative alt,
			JavaGenerationParameters params) {
	...
	private void genMakeMethod(Type type, Alternative alt, boolean forwarding,
			String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String altClassName = AlternativeGenerator.qualifiedClassName(params,
				type, alt);
		...
		if (!forwarding) {
			...
			println("};");
			println("    return " + makeMethodName + "(" + funVar
	...
	private void genInternalMakeMethod(Type type, Alternative alt,
		...
		if (!forwarding) {
			println("    return (" + altClassName + ") factory.build( new "
					+ altClassName + "(this, annos, fun, args));");
		} else { // forwarding
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ "." + methodName + "(fun, args, annos);");
		}
		println("  }");
		println();
	}

	private void genAltToTerm(Type type, Alternative alt, boolean forwarding,
			String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String className = AlternativeGenerator.qualifiedClassName(params,
				type, alt);

		println("  public aterm.ATerm toTerm(" + className + " arg) {");
		if (!forwarding) {
			println("    java.util.List<Object> args = new java.util.LinkedList<Object>();");

			Iterator<Field> fields = type.altFieldIterator(alt.getId());
			genAddFieldsToListCalls(fields);
			println("    return factory.make(" + patternVariable(type, alt)
					+ ", args);");
		} else {
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
	...
	private void genFactoryInitialization(ADT api) {
		...
		Iterator<String> bottoms = api.bottomTypeIterator();

		while (bottoms.hasNext()) {
			String type = bottoms.next();
	...
	private void genSeparatedListInitialization(int listTypeCount,
			SeparatedListType type) {
	...
	private void genInitializeEmptySeparatedList(SeparatedListType type,
			int listTypeCount) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String className = TypeGenerator.qualifiedClassName(params, type
				.getId());
	...
	private void genInitializeEmptyList(Type type, int listTypeCount) {
		...
		println("    " + emptyName + " = (" + className
				+ ") factory.build(new " + className
				+ "(this, factory.getEmpty(), null, null)" + ");");
	}
	...
	private void genAltFromTerm(Type type, Alternative alt) {
		...
		println("  protected " + returnType + " " + methodName
				+ "(aterm.ATerm trm) {");
		println("    java.util.List<Object> children = trm.match("
				+ patternVariable(type, alt) + ");");
		println();
	...
	private void genTypeFromTermMethod(Type type, boolean forwarding,
			String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		...
		println("  public " + returnType + " " + methodName
				+ "(aterm.ATerm trm) {");
		if (!forwarding) {
			println("    " + returnType + " tmp;");
			genAltFromTermCalls(type);
			println("    throw new IllegalArgumentException(\"This is not a "
					+ TypeGenerator.className(type) + ": \" + trm);");
		} else {// forwarding
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ "." + methodName + "(trm);");
	...
	protected void genTypeFromStringMethod(Type type) {
		...
		String returnType = TypeGenerator.qualifiedClassName(params, type
				.getId());
		String fromString = TypeGenerator.className(type.getId())
				+ "FromString";
	...
	protected void genTypeFromFileMethod(Type type) {
		...
		print("  public " + returnType + ' ' + fromFile
	...
	private void genListFromTerm(ListType type, boolean forwarding,
			String moduleName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		...
		String elementTypeName = TypeGenerator.qualifiedClassName(params,
				elementType);
		String nextElement;

		if (!getConverter().isReserved(elementType)) {
			nextElement = TypeGenerator.className(elementType)
					+ "FromTerm(list.getFirst())";
		} else {
			nextElement = getConverter().makeATermToBuiltinConversion(
					elementType, "list");
		}
		...
		if (!forwarding) {
			...
			println("        return result;");
			println("     }");
			println("     throw new RuntimeException(\"This is not a "
					+ className + ": \" + trm);");
		} else {// forwarding
			println("    return "
					+ FactoryGenerator.className(moduleName).toLowerCase()
					+ "." + className + "FromTerm(trm);");
		}
		println("  }");
		println();
	}
	...
	private void genSeparatedListFromTermMethod(SeparatedListType type,
		...
		if (!forwarding) {
			...
			String singletonElement = buildFromTermListElement(elementType,
					"list.getFirst()");
			...
			println("    }");
			println();
			println("    " + returnTypeName + " result = " + makeClassName
					+ "(" + singletonElement + ");");
			println();
			println("    for (int i = length - 2; i >= 0; i--) {");
			println("      java.util.List<Object> children = nodes[i];");
			String elementTypeName = TypeGenerator.qualifiedClassName(params,
					elementType);

			String head = buildFromTermListElement(elementType,
			...
			println("      result = " + makeClassName + "(head, "
					+ separatorArgs + "result);");
			println("    }");
			println();
			println("    return result;");
		} else {// forwarding
	...
	private String buildFromTermListElement(String elementType, String element) {
		if (getConverter().isReserved(elementType)) {
			element = getConverter().makeATermToBuiltinConversion(elementType,
					element);
		} else {
			String elementClass = TypeGenerator.className(elementType);
	...
	private void genSeparatedListToTerm(SeparatedListType type,
			boolean forwarding, String moduleName) {
		String paramTypeName = TypeGenerator.qualifiedClassName(
				getJavaGenerationParameters(), type.getId());
		String manyPattern = patternListVariable(type);
		String elementType = type.getElementType();
		...
		if (!forwarding) {
			...
			println();
			println("    int length = arg.getLength();");
			println("    " + paramTypeName + "[] nodes = new " + paramTypeName
					+ "[length];");
			println();
			println("    for (int i = 0; i < length; i++) {");
			...
			String convertedHead = getConverter().makeBuiltinToATermConversion(
					elementType, "nodes[0].getHead()");
			convertedHead += continueConversion;

			println("    aterm.ATerm result = factory.makeList("
					+ convertedHead + ");");
			println("    for (int i = 1; i < length; i++) {");
			println("      java.util.List<Object> args = new java.util.LinkedList<Object>();");

			String boxedHead = BoxingBuilder.buildBoxer(elementType,
					"nodes[i].getHead()");
	...
	public static String concatTypeAlt(Type type, Alternative alt) {
		return TypeGenerator.className(type) + "_"
				+ AlternativeGenerator.className(type, alt);
	}

	private static String alternativeVariable(String pre, Type type,
			Alternative alt) {
