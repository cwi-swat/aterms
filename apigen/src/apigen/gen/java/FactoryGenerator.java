...
public class FactoryGenerator extends JavaGenerator {
		...
		while (types.hasNext()) {
			...
			if (type instanceof ListType) {
				...
				if (type instanceof SeparatedListType) {
					...
					genMakeSingletonSeparatedList(returnTypeName, methodName, paramTypeName, sepListType, empty);
					genMakeManySeparatedList(
						...
						paramTypeName,
						sepListType);
					genMakeManySeparatedTermList(returnTypeName, methodName, proto, sepListType);
					genMakeFixedSizedSeparatedList(returnTypeName, methodName, sepListType);
	...
	private void genMakeFixedSizedSeparatedList(
		...
		String formalElems = buildFormalArgumentList(qualifiedElementName, "elem", size);
		String actualElems = buildActualArgumentList("elem", 1, size);
		println("  public " + returnTypeName + " " + methodName + "(" + formalSeps + formalElems + ") {");
	...
	private void genReverseSeparatedLists(SeparatedListType type, String makeMethodName) {
		JavaGenerationParameters params = getJavaGenerationParameters();
		String className = TypeGenerator.qualifiedClassName(params, type);
	...
	private void genMakeManyTermList(String returnTypeName, String methodName, String protoName) {
		println(
			"  protected "
				+ returnTypeName
				+ ' '
	...
	private void genMakeManyList(String paramType, String returnTypeName, String methodName, String paramTypeName) {
		String head;
	...
	private void genMakeManySeparatedList(
		...
		SeparatedListType type) {
		String formalSeps = buildFormalSeparatorArguments(type);
		String actualSeps = buildActualTypedAltArgumentListNoConversion(type.separatorFieldIterator());
		if (formalSeps.length() > 0) {
			formalSeps += ", ";
			actualSeps += ", ";
		}
	...
	private void genMakeSingletonList(String returnTypeName, String methodName, String paramTypeName, String empty) {
	...
	private void genMakeEmptyList(String returnTypeName, String methodName, String empty) {
	...
	private void genFactoryInitialization(ADT api) {
		...
		while (types.hasNext()) {
			...
			if (type instanceof ListType) {
				if (type instanceof SeparatedListType) {
					genSeparatedListInitialization(listTypeCount, (SeparatedListType) type);
				}
				else {
	...
	private void genSeparatedListInitialization(int listTypeCount, SeparatedListType type) {
		genInitializePrototype(type);
	...
	private void genListFromTerm(ListType type) {
		...
		if (!getConverter().isReserved(elementType)) {
			nextElement = TypeGenerator.className(elementType) + "FromTerm(list.getFirst())";
		}
		else {
	...
	private void genSeparatedListFromTermMethod(SeparatedListType type) {
		...
		println("    }");
		println();
		println("    " + returnTypeName + " result = " + makeClassName + "(" + singletonElement + ");");
		println();
