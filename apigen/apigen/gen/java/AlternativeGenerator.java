...
public class AlternativeGenerator extends JavaGenerator {
    ...
    private static String packageName(Type type) {
        return (TypeGenerator.packageName() + '.' + TypeGenerator.className(type))
            .toLowerCase();
    }
    ...
    private void genFieldSetterMethod(
        ...
        if (fieldType.equals("str")) {
            print(
                "getFactory().makeAppl(getFactory().makeAFun(" + fieldId + ", 0, true))");
    ...
    private void genFieldGetterMethod(
        String fieldName,
        String fieldType,
        String fieldClass,
        String fieldIndex) {
        ...
        if (fieldType.equals("str")) {
            println(
                "   return ((aterm.ATermAppl) getArgument("
                    + fieldIndex
                    + ")).getAFun().getName();");
        } else if (fieldType.equals("int")) {
            println(
                "   return ((aterm.ATermInt) getArgument(" + fieldIndex + ")).getInt();");
        } else if (fieldType.equals("real")) {
            println(
                "   return ((aterm.ATermReal) getArgument("
                    + fieldIndex
                    + ")).getReal();");
    ...
    private void genAltMake(Type type, Alternative alt) {
        ...
        println(
            "  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] args,"
                + " aterm.ATermList annos) {");
        println(
            "    return " + getFactoryMethod + "." + makeMethod + "(fun, args, annos);");
        println("  }");
        println();
    }
