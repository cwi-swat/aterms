...
#define TRM_STACK_REL(idx)	(term_stack_depth - (idx))
...
writeSymToFile(Symbol sym, FILE *f)
{
	...
	name = ATgetName(sym);
	writeStringToFile(name, strlen(name), f);
...
writeToBinaryFile(ATerm t, FILE *f)
{
	...
	switch(ATgetType(t))
	{
		...
		case AT_LIST:
			/* First write all the elements of the list */
...
ATwriteToBinaryFile(ATerm t, FILE *file)
{
	...
	/* Initialize stacks */
	term_stack_depth = 10;		/* Leave room for CMD_XXX */
